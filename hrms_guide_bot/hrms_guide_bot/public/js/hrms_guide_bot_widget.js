$(document).ready(function () {
    console.log("[HRMS Bot] Document ready. Initializing widget...");

    // Retry finding user if frappe.session isn't ready yet
    const initUserCheck = setInterval(() => {
        if (window.frappe && window.frappe.session) {
            clearInterval(initUserCheck);
            console.log("[HRMS Bot] Session detected. Checking user...");

            if (frappe.session.user === 'Guest') {
                console.log("[HRMS Bot] Guest user. Aborting.");
                return;
            }
            ...
            console.log("[HRMS Bot] Injecting widget...");
    ...
    }, 500); // Check for frappe.session every 500ms
});
